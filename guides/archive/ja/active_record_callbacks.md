...
コールバックの連鎖の全体は、1つのトランザクションに含まれます。_before_ コールバックの1つが`false`を返すか例外を発生するという動作をする場合、実行の連鎖全体が停止してROLLBACKが発行されます。_after_ コールバックの場合は例外を発生することによってのみ、コールバックの連鎖を停止することができます。

WARNING: `ActiveRecord::Rollback`以外のあらゆる例外は、コールバック連鎖がその例外によって停止した後に、Railsによって再び発生させられます。このため、コールバック連鎖の中で`ActiveRecord::Rollback`以外の例外が起こると、メソッドが例外を発生することを想定していないコード (通常時にtrueかfalseを返そうとする`save`や`update_attributes`メソッドなど) が壊れる恐れがあります。
