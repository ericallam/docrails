...
WARNING: `ActiveRecord::Rollback`以外のあらゆる例外は、コールバック連鎖がその例外によって停止した後に、Railsによって再び発生させられます。このため、コールバック連鎖の中で`ActiveRecord::Rollback`以外の例外が起こると、 `save`や`update_attributes`などの通常はtrueやfalseを返そうとするメソッド (例外は発生しないと想定しているコード) が壊れる恐れがあります。
