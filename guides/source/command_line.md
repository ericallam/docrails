...
`Rails.root/tmp`ディレクトリは、(*nix系でいう/tmpディレクトリのような) 一時ファイルを保存するためのディレクトリです。一時ファイルには、セッションのためのファイル(ファイルを利用してセッションの管理を行っている場合 (訳注: 最近のRailsではセッションをファイルで管理することは稀です))やプロセスIDのファイル、アクションキャッシュのためのファイルなどがあります。
