...
productionモードでは、Railsはデフォルトでプリコンパイルされたファイルを`public/assets`に置きます。このプリコンパイルされたファイルは、Webサーバーによって静的なアセットとして扱われます。`app/assets`に置かれたファイルがそのままの形でproduction環境で使用されることは決してありません。
