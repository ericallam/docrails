...
値のほかにBase64エンコーディングとメタデータも保存されるので、暗号化を利用する場合はカラムの容量を余分に必要とします。組み込みのエンベロープ暗号化キープロバイダが使われる場合、最悪で250バイトほど余分に必要になると見積もれます。これは中大規模のtextカラムでは無視できる量ですが、255バイトの`string`カラムではこれに応じて上限を増やしておく必要があります（推奨は510バイト）。
...
これは、暗号化済みデータと非暗号化データを組み合わせた場合や【原文のgitは誤り？】、以前の暗号化スキームを設定した場合にも利用できます。
...
2つのメソッドは、いずれも`ActiveRecord::Encryption::Key`オブジェクトを返します。
