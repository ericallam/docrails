...
たとえば「Simple」バックエンドを、複数のバックエンドをチェーンさせる「Chain」バックエンドに置き換えることもできます。これは、基本的には「Simple」バックエンドに保存された標準的な訳文を使いたいが、アプリケーションのカスタム訳文はデータベースなどの別のバックエンドに保存しておきたい、というような場合に便利です。
