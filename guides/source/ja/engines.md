...
WARNING: `require`は開発環境でのクラス自動読み込みの誤作動を起こすので使わないでください。`require_dependency`を使用することだけがクラスの読み込みされる、されないを保証する正しい使い方です。
...
しかしこれは正常に機能しないでしょう。アプリケーションは、このようなリクエストをエンジンにルーティングする方法を知らないので、明示的にエンジンにルーティングする必要があります。これを行なうには、設定こーどの中で`@routes`インスタンス変数にエンジンのルーティングを割り当てる必要があります。
