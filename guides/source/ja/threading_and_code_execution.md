...
各スレッドは、アプリケーションのコードを実行する前にこのようにラップされるべきです。これにより、アプリケーションで何らかの作業を他のスレッドに手動で委譲する場合（`Thread.new`を使うなど）や、Concurrent Rubyのスレッドプールを用いる場合は、そのブロックをただちに以下のようにラップすべきです。
...
Executorは安全に「[リエントラント](https://ja.wikipedia.org/wiki/%E3%83%AA%E3%82%A8%E3%83%B3%E3%83%88%E3%83%A9%E3%83%B3%E3%83%88)」できます。現在のスレッドで既にアクティブになっている場合、`wrap`は何も実行しません。
...
Railsフレームワークのコンポーネントでは、Rails自身で必要なコンカレンシーの管理にもこのツールが用いられています。
...
Action CableではReloaderではなくExecutorが使われます。Action Cableコネクションはクラスの特定のインスタンスに紐付けられていて、Websocketメッセージが到着するたびに再読み込みすることが不可能なためです。Action Cableではメッセージハンドラのみがラップされるので、長時間実行されるAction Cable接続でも、新しく到着したリクエストやジョブによってトリガされる再読み込みはブロックされません。クライアントが自動的に再接続すると、そのことが新バージョンのコードに伝わります。
...
Executorは、データベース接続の管理などの重要な作業を常に抱えています。`cache_classes`と`eager_load`がどちらも`true`（`production`モード）の場合、自動読み込みやクラスの再読み込みは発生しなくなるため、Load Interlockは不要になります。`cache_classes`と`eager_load`のいずれかが`false`（`development`モード）の場合、ExecutorはLoad Interlockを用いて安全な場合にのみ定数を読み込みます。
...
Rails.application.executor.wrap do
  ...
  th.join # 外側のスレッドは'running'をつかんだままここで待機する
end
```
