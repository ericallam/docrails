...
`:autosave`オプションを`true`に設定すると、親オブジェクトが保存されるたびに、読み込まれているすべての関連付けメンバを保存し、destroyフラグが立っているメンバを破棄します。`:autosave`を`false`に設定することと、`:autosave`を設定しないことは同じではない点にご注意ください。`:autosave`オプションがない場合、関連付けられた新規オブジェクトはsaveされますが、関連付けられた更新オブジェクトは保存されません。
...
`collection.build`メソッドは、関連付けが行われた新しいオブジェクトを単独または配列で返します。返されるオブジェクトは、渡された属性に基いてインスタンス化され、外部キーを経由するリンクが作成されます。関連付けられたオブジェクトは、値が返された時点ではまだ保存されて_いない_ことにご注意ください。
...
`collection.create`メソッドは、関連付けが行われた新しいオブジェクトを単独または配列で返します。このオブジェクトは、渡された属性を使用してインスタンス化され、そのオブジェクトの外部キーを介してリンクが作成されます。そして、関連付けられたモデルで指定されている検証がすべてパスすると、この関連付けられたオブジェクトは保存されます。
...
フィールドや振る舞いを異なるモデル間で共有したい場合があります。Carモデル、Motorcycleモデル、Bicycleモデルがある場合を考えてみましょう。このとき、`color`や`price`などのフィールドや、いくつかの関連メソッドを共有したいが、振る舞いやコントローラーがモデルごとに異なっているとします。
...
"type"というフィールドを追加している点にご注目ください。すべてのモデルがデータベース上の単独のテーブルに保存されるため、Railsは該当するモデル名をこのカラムに保存します。この例では、カラムには "Car"、"Motorcycle"、"Bicycle"のいずれかが保存されます。このシングルテーブル継承 (STI: Single Table Inheritance) では、テーブルにこの"type"フィールドがないと正常に動作しません。

次に、Vehicleモデルを継承して３つの各モデルを生成します。このとき、`--parent=PARENT`オプションを使って特定の親モデルを継承している点にご注目ください。このオプションを使うと、マイグレーションファイルは生成されません(該当するテーブルが既に存在しているため)。
...
これによって、Vehicleに追加されたすべての振る舞いがCarモデルにも追加されるようになります。関連付けやpublicメソッドなども同様に追加されます。
...
Carのレコードを取得するクエリを送信すると、実際にはvehiclesテーブル中のCarが検索されます。
