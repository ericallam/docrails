...
RubyGemsサイトでgemを公開する方法ついて詳しくは、[はじめてのRuby Gem作成・パブリッシュ方法](http://guides.rubygems.org/publishing)（英語）を参照してください。

RDocドキュメント
------------------

プラグインの開発がひと段落してデプロイする段階になったら、プラグインの利用者のためにちゃんとしたドキュメントを作成しましょう。幸い、プラグインのドキュメント作成は簡単です。
