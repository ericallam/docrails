...
INFO: 本ガイドでは、従来の`classic`モードの自動読み込みについて解説します。`zeitwerk`モードについては、[定数の自動読み込みと再読み込み（Zeitwerkモード）](autoloading_and_reloading_constants.html)を参照してください。
