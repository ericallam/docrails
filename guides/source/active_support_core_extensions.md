...
文字列に式展開すると、`:to`はメソッドが委譲されるオブジェクトに対して評価される式になります。通常は文字列またはシンボルになります。そのような式は、レシーバのコンテキストで評価されます。
...
Active Supportは、`Time.current`の値が`Time.zone.now`の値と同じになるように定義しています。ユーザータイムゾーンが定義されていれば`Time.now`と同じになります。
